      ...
      if (data.status === "success") {
        if (data.hosting) {
          result = {
            ip,
            isBlacklisted: true,
            reason: `Hosting facility IP (${data.org ?? data.isp})`,
            type: "hosting",
            cached: false,
          };
        } else if (data.proxy) {
          result = {
            ip,
            isBlacklisted: true,
            reason: "Anonymous VPN or proxy exit node",
            type: "vpn",
            cached: false,
          };
        }
...
export async function ipReputationMiddleware(
  ...
  try {
    ...
    if (reputation.isBlacklisted) {
      logger.warn(
        `Blocked transaction from blacklisted IP ${ip}: ${reputation.reason}`,
      );
      transactionErrorsTotal.inc({
        type: "ip_reputation",
        error_type: "blacklisted_ip",
      });
      res.status(403).json({
        error: "Forbidden",
        message:
          "Transaction rejected: your network has been flagged as high-risk.",
        code: "IP_BLACKLISTED",
      });
      return;
    }
...
const defaultConfig: FraudConfig = {
  maxTransactionsPerHour: parseInt(
    process.env.FRAUD_MAX_TRANSACTIONS_PER_HOUR || "5",
  ),
  maxTransactionsPerMinute: parseInt(
    process.env.FRAUD_MAX_TRANSACTIONS_PER_MINUTE || "2",
  ),
  ...
  timeWindowMs: parseInt(
    process.env.FRAUD_TIME_WINDOW_MS || `${60 * 60 * 1000}`,
  ),
  shortTimeWindowMs: parseInt(
    process.env.FRAUD_SHORT_TIME_WINDOW_MS || `${5 * 60 * 1000}`,
  ),
  ...
  rapidSuccessionScore: parseInt(
    process.env.FRAUD_RAPID_SUCCESSION_SCORE || "20",
  ),
  amountScore: parseInt(process.env.FRAUD_AMOUNT_SCORE || "20"),
  geoScore: parseInt(process.env.FRAUD_GEO_SCORE || "20"),
  highRiskNumberScore: parseInt(
    process.env.FRAUD_HIGH_RISK_NUMBER_SCORE || "30",
  ),
  ...
  frequencySpikeScore: parseInt(
    process.env.FRAUD_FREQUENCY_SPIKE_SCORE || "15",
  ),
  ...
  highValueThreshold: parseFloat(
    process.env.FRAUD_HIGH_VALUE_THRESHOLD || "1000",
  ),
  newAccountDays: parseInt(process.env.FRAUD_NEW_ACCOUNT_DAYS || "7"),
  deviceFingerprintWindowMs: parseInt(
    process.env.FRAUD_DEVICE_FINGERPRINT_WINDOW_MS || `${24 * 60 * 60 * 1000}`,
  ),
...
export class FraudService {
    ...
    try {
      ...
      if (!existingDevices.includes(deviceFingerprint)) {
        ...
        await redisClient.expire(
          key,
          this.config.deviceFingerprintWindowMs / 1000,
        );
        return true;
      }
      return false;
  ...
  private detectFrequencySpike(
    ...
    const olderTxns = transactions.filter(
      (t) =>
        now.getTime() - new Date(t.createdAt).getTime() > 24 * 60 * 60 * 1000 &&
  ...
  async detectFraud(
    ...
    const sortedTxns = [...userTransactions].sort(
      (a, b) =>
    ...
    if (recentTxns.length >= this.config.maxTransactionsPerHour) {
      ...
      reasons.push(
        `Too many transactions (${recentTxns.length}) in ${this.config.timeWindowMs / (60 * 60 * 1000)} hours`,
      );
    ...
    if (rapidTxns.length >= this.config.maxTransactionsPerMinute) {
      ...
      reasons.push(
        `Rapid succession transactions (${rapidTxns.length}) in 1 minute`,
      );
    ...
    if (transactionInput.amount > avgAmount * this.config.amountMultiplier) {
      ...
      reasons.push(
        `Unusually large amount ($${transactionInput.amount} vs avg $${avgAmount.toFixed(2)})`,
      );
    ...
    if (transactionInput.location) {
      const lastTxnWithLocation = sortedTxns.find(
      ...
      if (lastTxnWithLocation && lastTxnWithLocation.locationMetadata) {
        ...
        const timeDiff =
          now.getTime() - new Date(lastTxnWithLocation.createdAt).getTime();
        if (
          ...
          reasons.push(
            `Suspicious location change (${distance.toFixed(2)}km in ${timeDiff / (60 * 60 * 1000)} hours)`,
          );
    ...
    if (transactionInput.ipAddress && user) {
      ...
      if (
        ipLocation &&
        this.isLocationMismatch(ipLocation, transactionInput.location)
      ) {
    ...
    if (
      hour >= this.config.unusualHoursStart ||
      hour <= this.config.unusualHoursEnd
    ) {
    ...
    if (failedAttempts.length >= 3) {
      ...
      reasons.push(
        `Multiple failed attempts (${failedAttempts.length}) in short time`,
      );
    ...
    if (user) {
      ...
      if (
        daysOld <= this.config.newAccountDays &&
        transactionInput.amount >= this.config.highValueThreshold
      ) {
        score += this.config.newAccountScore;
        reasons.push(
          `High-value transaction from new account (${daysOld.toFixed(1)} days old)`,
        );
      ...
      if (
        user.kycLevel &&
        this.isLowKYCLevel(user.kycLevel) &&
        transactionInput.amount >= this.config.highValueThreshold
      ) {
        score += this.config.kycRiskScore;
        reasons.push(
          `High-value transaction from low KYC level (${user.kycLevel})`,
        );
    ...
    if (transactionInput.ipAddress) {
      try {
        const ipReputation = await checkIPReputation(
          transactionInput.ipAddress,
        );
        if (ipReputation.isBlacklisted) {
          score += this.config.ipReputationScore;
          reasons.push(
            `Blacklisted IP address: ${ipReputation.reason} (type: ${ipReputation.type})`,
          );
          heuristicsTriggered.push("ip_reputation");
        }
      } catch (err) {
        logger.warn(
          `IP reputation check failed for ${transactionInput.ipAddress}:`,
          err,
        );
      }
    }
  ...
  async setTransactionToReview(transactionId: string): Promise<void> {
    try {
      ...
      logger.error(
        `Failed to set transaction ${transactionId} to Review:`,
        error,
      );
      throw error;
    }
