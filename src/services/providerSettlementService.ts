...
export class ProviderSettlementService {
  ...
  async runDailySettlement(
    ...
    for (const agg of aggregates) {
      ...
      if (record.status === "settled") {
        ...
        summary.issues.push(`[${record.provider}] ${record.errorMessage}`);
      }
    }
  ...
  private async settleProvider(
    ...
    const base: Omit<
      SettlementRecord,
      "id" | "createdAt" | "status" | "errorMessage" | "ledgerReference"
    > = {
      settlementDate: dateStr,
    ...
    if (
      ...
      (agg.merchantFeeTotal === 0 && agg.providerFeeTotal === 0)
    ) {
      console.log(
        `[settlement] Skipping ${agg.provider} — no activity on ${dateStr}`,
      );
...
export const providerSettlementService = new ProviderSettlementService();
