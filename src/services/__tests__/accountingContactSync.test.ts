  ...
  describe("Xero contact sync", () => {
    it("reuses an existing Xero contact matched by email", async () => {
      ...
      mockAxios.get.mockResolvedValue({
        data: {
          Contacts: [
            { ContactID: "contact-123", EmailAddress: "alice@example.com" },
          ],
        },
      });
      // 5: insert mapping
      ...
      expect(mockPool.query).toHaveBeenCalledWith(
        expect.stringContaining("INSERT INTO accounting_contact_mappings"),
        expect.any(Array),
      );
    });
    ...
    it("creates a new Xero contact when none match the email", async () => {
      ...
      mockAxios.post.mockResolvedValue({
        data: { Contacts: [{ ContactID: "new-contact-1" }] },
      });
        ...
        data: {
          QueryResponse: {
            Customer: [
              {
                Id: "qb-customer-123",
                BillAddr: { Email: "charlie@example.com" },
              },
            ],
          },
        },
