...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  private isSupportedCountry(phoneNumber: string): boolean {
    ...
    return (
      trimmed.startsWith("+229") ||
      trimmed.startsWith("+228") ||
  ...
  async requestPayment(
    ...
    if (!this.isSupportedCountry(phoneNumber)) {
      const errorMsg =
        "Moov Money only supports Benin (+229), Togo (+228), and Côte d'Ivoire (+225) phone numbers";
    ...
    try {
      ...
      const requestXml = this.buildSoapEnvelope(
        "RequestPayment",
        bodyContent,
        signature,
      );
      ...
      const signatureMatch = responseXml.match(
        /<Signature[^>]*>([^<]+)<\/Signature>/,
      );
      if (!signatureMatch) {
        throw new Error(
          "Response signature verification failed: SOAP response is missing Signature header",
        );
      }
      ...
      const transactionId = this.getXmlElementValue(
        responseXml,
        "TransactionId",
      );
      ...
      if (status === "SUCCESS" || status === "PENDING") {
        ...
        const errorDetail =
          this.getXmlElementValue(responseXml, "ErrorDetail") ||
  ...
  async getTransactionStatus(
    ...
    try {
      ...
      log.error(
        { referenceId, error: error.message },
        "Moov: Status query failed",
      );
      return { status: "unknown" };
    }
  }
