...
export class VodacomProvider {
  ...
  constructor() {
    ...
    this.serviceProviderCode =
      process.env.VODACOM_SERVICE_PROVIDER_CODE || "000000";
    this.baseUrl =
  ...
  private async getAccessToken(): Promise<string> {
    ...
    if (!this.apiKey || !this.publicKey) {
      throw new Error(
        "Vodacom Provider: VODACOM_API_KEY and VODACOM_PUBLIC_KEY must be configured",
      );
    }

    const encryptedKey = encrypt(this.apiKey, this.publicKey);

    const response = await this.client.get(`/${this.market}/getSession/`, {
      headers: {
    ...
    if (responseCode !== "INS-0" || !sessionID) {
      throw new Error(
        `Vodacom getSession failed with code ${responseCode}: ${response.data?.output_ResponseDesc || "Unknown error"}`,
      );
    }
    ...
    try {
      ...
      if (code === "INS-0") {
        log.info(
          ...
          "Vodacom: Payment request successful",
        );
        return {
          success: true,
          data: response.data,
    ...
    try {
      ...
      if (code === "INS-0") {
        log.info(
          ...
          "Vodacom: Payout request successful",
        );
        return {
          success: true,
          data: response.data,
