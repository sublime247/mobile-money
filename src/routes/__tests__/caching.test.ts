...
import {
  ...
  CacheTags,
  QUERY_TTL_POLICIES,
} from "../../services/cachedQueryManager";
import {
  TransactionCacheInvalidation,
  WebhookCacheInvalidation,
  CacheKeyGenerators,
} from "../../services/cacheAside";
...
describe("Cache Query Manager", () => {
  ...
  describe("Cache-aside pattern", () => {
    it("should fetch from source on cache miss", async () => {
      ...
      const result = await cachedQueryManager.getOrFetch(cacheKey, fetchFn, {
        ttlSeconds: 300,
        tags: ["test"],
      });
  ...
  describe("TTL policies", () => {
    ...
    it("should have longer TTL for less frequent queries", () => {
      expect(QUERY_TTL_POLICIES.PRICE_HISTORY).toBeGreaterThan(
  ...
  describe("Tag-based invalidation", () => {
    it("should invalidate all caches with a specific tag", async () => {
      ...
      jest
        .mocked(cachedQueryManager["redis"].smembers as any)
...
describe("Transaction Cache Invalidation", () => {
  let invalidateByTagsSpy: jest.SpiedFunction<
    typeof cachedQueryManager.invalidateByTags
  >;
  let invalidateByTagSpy: jest.SpiedFunction<
    typeof cachedQueryManager.invalidateByTag
  >;

  beforeEach(() => {
    jest.clearAllMocks();
    invalidateByTagsSpy = jest
      .spyOn(cachedQueryManager, "invalidateByTags")
      .mockResolvedValue(0);
    invalidateByTagSpy = jest
      .spyOn(cachedQueryManager, "invalidateByTag")
      .mockResolvedValue(0);
  });
...
describe("Cache performance", () => {
  it("should improve performance for repeat queries", async () => {
    ...
    await cachedQueryManager.getOrFetch(cacheKey, fetchFn, {
      ttlSeconds: 300,
      tags: ["test"],
    });
