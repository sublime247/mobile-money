...
router.post("/use", async (req: Request, res: Response) => {
  ...
  if (!userId || !referral_code)
    return res.status(400).json({ error: "Missing userId or referral_code" });
  try {
    // Validate code
    const ref = await referralModel.findByCode(referral_code);
    if (!ref || ref.user_id === userId)
      return res.status(400).json({ error: "Invalid or self-referral code" });
    // Prevent double-spend
    if (await referralModel.hasUsedReferral(userId))
