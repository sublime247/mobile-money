...
export class AccountingSyncJob {
  ...
  private async runDailySync(): Promise<void> {
    ...
    try {
      ...
      for (const connection of connections) {
        try {
          logger.info(
            `Syncing P&L for connection ${connection.id} (${connection.provider})`,
          );

          const syncLog = await this.accountingService.syncDailyPnL(
            connection.id,
            dateStr,
          );
          ...
          if (syncLog.status === "completed") {
            ...
            logger.error(
              `P&L sync failed for connection ${connection.id}: ${syncLog.errorMessage}`,
            );
          }
        } catch (error) {
          logger.error(
            `Error syncing P&L for connection ${connection.id}:`,
            error,
          );
        }
      }
  ...
  private async runFeeRevenueSync(): Promise<void> {
    ...
    try {
      ...
      for (const connection of connections) {
        try {
          logger.info(
            `Syncing fee revenue for connection ${connection.id} (${connection.provider})`,
          );

          const syncLog = await this.accountingService.syncFeeRevenue(
            connection.id,
            today,
          );
          ...
          if (syncLog.status === "completed") {
            logger.info(
              `Fee revenue sync completed for connection ${connection.id}`,
            );
          } else {
            logger.error(
              `Fee revenue sync failed for connection ${connection.id}: ${syncLog.errorMessage}`,
            );
          }
        } catch (error) {
          logger.error(
            `Error syncing fee revenue for connection ${connection.id}:`,
            error,
          );
        }
      }
  ...
  private async getAllActiveConnections(): Promise<
