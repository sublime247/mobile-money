...
export async function runAccountingReconciliationJob() {
  logger.info(
    "[accounting-reconciliation-job] Starting daily accounting reconciliation",
  );
  ...
  try {
    ...
    logger.info(
      "[accounting-reconciliation-job] Daily accounting reconciliation job completed successfully",
    );
  } catch (error) {
    logger.error(
      error,
      "[accounting-reconciliation-job] Daily accounting reconciliation job failed",
    );
  }
}
