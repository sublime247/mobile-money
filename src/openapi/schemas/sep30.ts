...
export const StellarPublicKeySchema = z.string().length(56).openapi({
  example: "GABC...XYZ",
...
export const RecoverySessionSchema = registry.register(
  ...
  z
    .object({
      ...
      approvedBy: z.array(StellarPublicKeySchema).openapi({
        description:
          "List of signer public keys that have submitted valid signatures",
      }),
      requestedNewAddress: StellarPublicKeySchema.nullable()
        .optional()
        .openapi({
          description:
            "Optional custom Stellar address to rotate to. NULL = generate fresh keypair.",
        }),
      expiresAt: z
        .string()
        .datetime()
        .openapi({ example: "2024-04-25T10:30:00.000Z" }),
    ...
    .openapi("Sep30RecoverySession", {
      description:
        "Multi-sig key recovery session tracking state, approvals, and audit info.",
    }),
);
...
export const ManagedKeySchema = registry.register(
  ...
  z
    ...
    .openapi("Sep30ManagedKey", {
      description: "An AES-256-GCM-encrypted Stellar keypair.",
    }),
);
  ...
  z
    .object({
      requestedNewAddress: StellarPublicKeySchema.optional().openapi({
        description:
          "Optional: rotate to a specific Stellar address instead of generating a fresh one.",
      }),
    })
  ...
  z
    .object({
      ...
      token: z
        .string()
        .min(1)
        .openapi({ description: "Raw token from the /initiate response." }),
      signature: z.string().min(1).openapi({
        description:
          "Base64 Stellar signature of the token bytes made with the signer's private key.",
      }),
    })
