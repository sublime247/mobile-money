...
async function fetchSARData(
  ...
  // Fetch recent alerts
  const alerts = amlService
    .getAlerts({ userId })
    .filter((a) => new Date(a.createdAt) >= ninetyDaysAgo);

  const alertContext = alertId
  ...
  const riskFlags = Array.from(
    new Set(alerts.flatMap((a) => a.ruleHits.map((h) => h.rule))),
  );
...
async function generatePDFBuffer(data: SARData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc
      .fillColor("#2c3e50")
      .fontSize(20)
      .text("MOBILE MONEY PLATFORM", { align: "left" });
    doc
      .fillColor("#7f8c8d")
      .fontSize(10)
      .text("Compliance and Regulatory Affairs", { align: "left" });
    doc.moveDown();

    doc
      .fillColor("#c0392b")
      .fontSize(16)
      .text("SUSPICIOUS ACTIVITY REPORT (SAR)", {
        align: "center",
        underline: true,
      });
    doc.moveDown();
    ...
    doc.text(
      `Generated: ${data.summary.reportDate.toLocaleString()}`,
      350,
      top,
    );
    doc.moveDown(2);
    ...
    doc
      .fillColor("#2980b9")
      .fontSize(14)
      .text("SECTION I: SUBJECT INFORMATION", { underline: true });
    ...
    doc
      .fillColor("#2980b9")
      .fontSize(14)
      .text("SECTION II: ACTIVITY SUMMARY", { underline: true });
    doc.fillColor("#000").fontSize(11);
    doc.moveDown(0.5);
    doc.text(`Detection Period: Last 90 Days`);
    doc.text(`Total Transactions Evaluated: ${data.summary.totalTransactions}`);
    doc.text(
      `Aggregated Suspicious Volume: ${data.summary.totalAmount.toLocaleString()} XAF`,
    );
    doc.moveDown(0.5);
    ...
    doc
      .fillColor("#2980b9")
      .fontSize(14)
      .text("SECTION III: NARRATIVE DESCRIPTION", { underline: true });
    doc.fillColor("#000").fontSize(11);
    doc.moveDown(0.5);
    ...
    doc
      .fillColor("#2980b9")
      .fontSize(14)
      .text("SECTION IV: TRANSACTION LOG (SAMPLED)", { underline: true });
    doc.moveDown(0.5);
    ...
    if (data.transactions.length > 25) {
      doc
        ...
        .fontSize(9)
        .text(
          `Note: Only showing first 25 of ${data.transactions.length} total transactions. Full history available in XML export.`,
          50,
          currentY + 10,
        );
    }
...
async function storeSARFile(
  ...
  if (!result.success || !result.fileUrl) {
    throw new Error(
      `Failed to store SAR ${type}: ${result.error || "Unknown error"}`,
    );
  }
...
export async function generateSAR(
  userId: string,
  alertId?: string,
): Promise<{ pdfUrl: string; xmlUrl: string }> {
  try {
    // 1. Collate data
    ...
    console.log(
      `SAR reports generated for user ${userId}. PDF: ${pdfUrl}, XML: ${xmlUrl}`,
    );
