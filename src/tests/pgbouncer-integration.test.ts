...
describe("PgBouncer Integration Tests", () => {
  ...
  describe("Query Operations", () => {
    ...
    it("should handle parameterized queries safely", async () => {
      const result = await queryRead("SELECT $1::TEXT as value", [
        "safe_value",
      ]);
  ...
  describe("Performance Metrics", () => {
    it("should measure query latency through PgBouncer", async () => {
    ...
    it("should show throughput improvement", async () => {
      const iterations = 30;
      const start = Date.now();
      ...
      for (let i = 0; i < iterations; i += batchSize) {
        const batch = Array.from(
          { length: Math.min(batchSize, iterations - i) },
          () => pool.query("SELECT $1 as num", [Math.random()]),
        );
  ...
  describe("Acceptance Criteria Verification", () => {
    ...
    it("should improve overall throughput", async () => {
    ...
    it("should maintain sub-100ms latency for typical queries", async () => {
      ...
      console.log(
        `Average latency: ${avgLatency.toFixed(2)}ms, P99: ${p99Latency}ms`,
      );
