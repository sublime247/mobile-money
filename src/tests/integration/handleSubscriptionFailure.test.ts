...
const mockedNotification = notificationRouter as jest.Mocked<
...
describe("handleSubscriptionFailure", () => {
  ...
  test("schedules retry when under max_retries", async () => {
    mockedSub.getById = jest
      .fn()
      .mockResolvedValueOnce({ id: "sub1", merchant_id: "m1" } as any);
    mockedSub.incrementRetry = jest.fn().mockResolvedValueOnce({
      retry_count: 1,
      max_retries: 3,
    ...
    expect(mockedSub.recordAttempt).toHaveBeenCalledWith(
      "sub1",
      "tx1",
      1,
      "failed",
  ...
  test("pauses and notifies when max_retries reached", async () => {
    mockedSub.getById = jest
      .fn()
      .mockResolvedValueOnce({ id: "sub2", merchant_id: "m2" } as any);
    mockedSub.incrementRetry = jest.fn().mockResolvedValueOnce({
      retry_count: 5,
      max_retries: 5,
      retry_backoff_seconds: 10,
    ...
    mockedNotification.routeSystemNotification = jest
      .fn()
