...
describe("ApiKey scopes and helpers", () => {
  ...
  test("listAllScopeNames includes known scopes", () => {
    ...
    expect(all).toEqual(
      expect.arrayContaining(["DEPOSITS_INITIATE", "TRANSACTIONS_READ"]),
    );
  });
  ...
  test("createApiKey honors named scopes and sets permissions bitmask", () => {
    ...
    expect(key.scopes).toEqual(
      expect.arrayContaining(["DEPOSITS_INITIATE", "DEPOSITS_READ"]),
    );
    expect(
      (key.permissions & ApiKeyScope.DEPOSITS_INITIATE) ===
        ApiKeyScope.DEPOSITS_INITIATE,
    ).toBe(true);
    expect(
      (key.permissions & ApiKeyScope.DEPOSITS_READ) ===
  ...
  test("describeScopes returns names for a permission bitmask", () => {
    ...
    expect(names).toEqual(
      expect.arrayContaining(["TRANSACTIONS_READ", "BALANCE_READ"]),
    );
  });
  ...
  test("validateTimeWindow catches invalid values and accepts valid windows", () => {
    ...
    expect(validateTimeWindow({ startHour: 1, endHour: 1 })).toBe(
      "startHour and endHour must differ",
    );
