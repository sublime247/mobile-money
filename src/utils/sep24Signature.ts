...
const WEBVIEW_URL_EXPIRATION_SECONDS = parseInt(
  process.env.WEBVIEW_URL_EXPIRATION_SECONDS || "600",
  10,
); // Default 10 minutes
...
if (!WEBVIEW_SIGNING_SECRET && process.env.NODE_ENV === "production") {
  logger.error(
    "WEBVIEW_SIGNING_SECRET is not set. This is critical for SEP-24 webview security.",
  );
 ...
 */
export function generateSignedSep24Url(
  baseUrl: string,
  params: Sep24UrlParams,
): string {
  if (!WEBVIEW_SIGNING_SECRET) {
    throw new Error(
      "WEBVIEW_SIGNING_SECRET is not configured. Cannot sign SEP-24 URL.",
    );
  }
...
export function verifySep24Signature(
  ...
  if (!WEBVIEW_SIGNING_SECRET) {
    logger.error(
      "WEBVIEW_SIGNING_SECRET is not configured. Cannot verify SEP-24 URL signature.",
    );
    return res
      .status(500)
      .json({ error: "Server configuration error: Signing secret missing." });
  }
  ...
  if (!sig || typeof sig !== "string") {
    logger.warn(
      { url: req.originalUrl },
      "SEP-24 Webview: Missing or invalid signature parameter",
    );
  ...
  if (!exp || typeof exp !== "string") {
    logger.warn(
      { url: req.originalUrl },
      "SEP-24 Webview: Missing or invalid expiration timestamp",
    );
    return res
      .status(400)
      .json({ error: "Missing or invalid expiration timestamp" });
  }

  const expirationTimestamp = parseInt(exp, 10);
  if (isNaN(expirationTimestamp) || expirationTimestamp <= 0) {
    logger.warn(
      { url: req.originalUrl, exp },
      "SEP-24 Webview: Invalid expiration timestamp format",
    );
    return res
      .status(400)
      .json({ error: "Invalid expiration timestamp format" });
  }
  ...
  if (now > expirationTimestamp) {
    logger.warn(
      { url: req.originalUrl, exp },
      "SEP-24 Webview: Expired URL access attempt",
    );
