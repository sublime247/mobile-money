...
export class CurrencyConfig {
  ...
  private static initialize(config?: CurrencyConfiguration): void {
    ...
    try {
      ...
      if (this.initializationErrors.length > 0) {
        console.warn(
          "Currency configuration initialized with warnings:",
          this.initializationErrors,
        );
      }
    } catch (error) {
  ...
  private static validateConfiguration(config: CurrencyConfiguration): void {
    ...
    for (const [code, rule] of Object.entries(config.currencies)) {
      try {
        ...
        this.initializationErrors.push(
          `Invalid currency rule for ${code}: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }
  ...
  private static validateCurrencyRule(rule: CurrencyRule, code: string): void {
    ...
    if (!["round", "floor", "ceil"].includes(rule.formatting.roundingMode)) {
      throw new Error(
        "Currency rule must have valid rounding mode (round, floor, or ceil)",
      );
    }
    ...
    if (
      typeof rule.validation.precision !== "number" ||
      rule.validation.precision < 0
    ) {
  ...
  private static validateSettings(
    settings: CurrencyConfiguration["settings"],
  ): void {
    if (
      ...
      this.initializationErrors.push(
        "Invalid defaultCurrency in settings, using USD",
      );
    ...
    if (!settings.defaultLocale || typeof settings.defaultLocale !== "string") {
      this.initializationErrors.push(
        "Invalid defaultLocale in settings, using en-US",
      );
    ...
    if (typeof settings.cacheSize !== "number" || settings.cacheSize <= 0) {
      this.initializationErrors.push(
        "Invalid cacheSize in settings, using default 100",
      );
      console.warn("Invalid cacheSize in settings, using default 100");
    }

    if (
      typeof settings.performanceThreshold !== "number" ||
      settings.performanceThreshold <= 0
    ) {
      this.initializationErrors.push(
        "Invalid performanceThreshold in settings, using default 10ms",
      );
      console.warn(
        "Invalid performanceThreshold in settings, using default 10ms",
      );
   ...
   */
  private static createSafeConfiguration(
    ...
    for (const [code, rule] of Object.entries(config.currencies)) {
      try {
        ...
        this.initializationErrors.push(
          `Skipped invalid currency ${code}: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }
    ...
    if (config.settings) {
      safeConfig.settings = {
        defaultCurrency:
          typeof config.settings.defaultCurrency === "string" &&
          config.settings.defaultCurrency
            ? config.settings.defaultCurrency
            : "USD",
        defaultLocale:
          typeof config.settings.defaultLocale === "string" &&
          config.settings.defaultLocale
            ? config.settings.defaultLocale
            : "en-US",
        cacheSize:
          typeof config.settings.cacheSize === "number" &&
          config.settings.cacheSize > 0
            ? config.settings.cacheSize
            : 100,
        performanceThreshold:
          typeof config.settings.performanceThreshold === "number" &&
          config.settings.performanceThreshold > 0
  ...
  static getCurrencyRule(currencyCode: string): CurrencyRule {
    ...
    if (!rule) {
      throw new Error(
        `Unsupported currency code: ${currencyCode}. Supported currencies: ${Object.keys(this.configuration.currencies).join(", ")}`,
      );
    }
  ...
  static addCurrency(rule: CurrencyRule): void {
    ...
    if (normalizedCode in this.configuration.currencies) {
      throw new Error(
        `Currency ${normalizedCode} already exists. Use updateConfiguration to modify existing currencies.`,
      );
    }
  ...
  static updateConfiguration(config: Partial<CurrencyRule>[]): void {
    ...
    for (const update of config) {
      ...
      if (
        update.numericCode !== undefined &&
        (typeof update.numericCode !== "number" || update.numericCode <= 0)
      ) {
        throw new Error(`Invalid numericCode for ${update.code}`);
      }

      if (
        update.minorUnits !== undefined &&
        (typeof update.minorUnits !== "number" || update.minorUnits < 0)
      ) {
      ...
      if (
        ...
        (!update.symbol || typeof update.symbol !== "string")
      ) {
      ...
      if (update.formatting) {
        if (
          update.formatting.locale !== undefined &&
          (!update.formatting.locale ||
            typeof update.formatting.locale !== "string")
        ) {
          throw new Error(`Invalid locale for ${update.code}`);
        }

        if (
          update.formatting.style !== undefined &&
          update.formatting.style !== "currency"
        ) {
          throw new Error(
            `Invalid style for ${update.code}: must be "currency"`,
          );
        }

        if (
          update.formatting.useGrouping !== undefined &&
          typeof update.formatting.useGrouping !== "boolean"
        ) {
          throw new Error(`Invalid useGrouping for ${update.code}`);
        }

        if (
          update.formatting.roundingMode !== undefined &&
          !["round", "floor", "ceil"].includes(update.formatting.roundingMode)
        ) {
      ...
      if (update.validation) {
        if (
          update.validation.minValue !== undefined &&
          typeof update.validation.minValue !== "number"
        ) {
          throw new Error(`Invalid minValue for ${update.code}`);
        }

        if (
          update.validation.maxValue !== undefined &&
          typeof update.validation.maxValue !== "number"
        ) {
          throw new Error(`Invalid maxValue for ${update.code}`);
        }

        if (
          update.validation.precision !== undefined &&
          (typeof update.validation.precision !== "number" ||
            update.validation.precision < 0)
        ) {
    ...
    for (const [code, update] of Object.entries(updates)) {
      ...
      if (updatedRule.validation.minValue >= updatedRule.validation.maxValue) {
        throw new Error(
          `After update, minValue must be less than maxValue for ${code}`,
        );
      }
    }
  }
  ...
  static isValidAmount(amount: number, currencyCode: string): boolean {
    ...
    try {
      ...
      return (
        amount >= rule.validation.minValue &&
