...
pub const DEFAULT_MAX_CLAIM_AMOUNT: i128 = 10_000_000_0000;
...
mod tests {
    ...
    const INITIAL_FAUCET_BALANCE: i128 = 1_000_000_000_0000; // 100,000 tokens
    const CLAIM_AMOUNT: i128 = 10_000_000_0000; // 10,000 tokens
    ...
    fn test_successful_claim() {
        ...
        assert_eq!(client.get_last_claim(&user, &token), Some(env.ledger().timestamp()));
        assert!(!client.can_claim(&user, &token));
    }
    ...
    fn test_admin_configuration_updates_and_withdraw() {
        ...
        client.withdraw(&token, &destination, &100_000_0000);

        let token_client = token::Client::new(&env, &token);
        assert_eq!(token_client.balance(&destination), 100_000_0000);
    }
}
