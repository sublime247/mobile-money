...
describe("TransactionLimitService", () => {
  ...
  beforeEach(() => {
    ...
    mockTransactionModel =
      new TransactionModel() as jest.Mocked<TransactionModel>;
  ...
  describe("checkTransactionLimit", () => {
    ...
    it("should include upgrade suggestion for Unverified users", async () => {
      ...
      expect(result.message).toContain(
        "Upgrade to Basic KYC for 100,000 XAF daily limit",
      );
      expect(result.upgradeAvailable).toBe(true);
    });
    ...
    it("should include upgrade suggestion for Basic users", async () => {
      ...
      expect(result.allowed).toBe(false);
      expect(result.message).toContain(
        "Upgrade to Full KYC for 1,000,000 XAF daily limit",
      );
      expect(result.upgradeAvailable).toBe(true);
    });
    ...
    it("should query transactions from 24-hour window", async () => {
      ...
      expect(
        mockTransactionModel.findCompletedByUserSince,
      ).toHaveBeenCalledWith(userId, expect.any(Date));

      const callDate = mockTransactionModel.findCompletedByUserSince.mock
        .calls[0][1] as Date;
