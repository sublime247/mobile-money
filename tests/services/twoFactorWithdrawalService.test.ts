import {
  ...
  twoFactorWithdrawalService,
} from "../../src/services/twoFactorWithdrawalService";
...
describe("TwoFactorWithdrawalService", () => {
  ...
  describe("requires2FAForWithdrawal", () => {
    ...
    it("throws when user not found", async () => {
      ...
      await expect(svc.requires2FAForWithdrawal("missing")).rejects.toThrow(
        "User not found",
      );
    });
  });
  ...
  describe("getWithdrawal2FASettings", () => {
    ...
    it("throws when user not found", async () => {
      mockFindById.mockResolvedValue(null);
      await expect(svc.getWithdrawal2FASettings("missing")).rejects.toThrow(
        "User not found",
      );
    });
  });
  ...
  describe("updateMandatory2FAWithdrawals", () => {
    it("throws when user not found", async () => {
      mockFindById.mockResolvedValue(null);
      await expect(
        svc.updateMandatory2FAWithdrawals("missing", true),
    ...
    it("throws when enabling without 2FA set up", async () => {
      ...
      await expect(
        svc.updateMandatory2FAWithdrawals("user-1", true),
      ).rejects.toThrow(
...
describe("validate2FAForWithdrawal middleware", () => {
  ...
  it("calls next() when user does not require 2FA", async () => {
    jest
      .spyOn(twoFactorWithdrawalService, "requires2FAForWithdrawal")
  ...
  it("returns 401 when 2FA required but verification fails", async () => {
    ...
    jest
      .spyOn(twoFactorWithdrawalService, "verifyWithdrawal2FA")
      .mockResolvedValue({ success: false, error: "Invalid token" });
    const res = makeRes();
    await validate2FAForWithdrawal(
      makeReq({ otpToken: "bad" }, "user-1"),
      res,
      next,
    );
    expect(res.status).toHaveBeenCalledWith(401);
  ...
  it("calls next() when 2FA verification succeeds", async () => {
    ...
    jest
      .spyOn(twoFactorWithdrawalService, "verifyWithdrawal2FA")
      .mockResolvedValue({ success: true, method: "totp" });
    const res = makeRes();
    await validate2FAForWithdrawal(
      makeReq({ otpToken: "123456" }, "user-1"),
      res,
      next,
    );
    expect(next).toHaveBeenCalled();
