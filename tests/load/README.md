...
# Peak-Day Traffic Spike — spikes rapidly to 5 000 VUs in 10 seconds
npm run test:load:traffic-spike
...
| `peak`                       | **1 000**  | ~18 min    | Rated capacity target _(default)_                                     |
...
| `traffic_spike` / `spike_5k` | **5 000**  | ~30s       | Sudden peak-day traffic spike (5K req in 10s)                         |
...
### 5K / 10K+ ingestion stress profiles

The `ingestion_10k`, `spike_10k`, `traffic_spike`, and `spike_5k` profiles execute only transaction-write requests against `/api/v1/transactions/deposit` and `/api/v1/transactions/withdraw`. This keeps the full 5K/10K+ virtual-user pressure on the ingestion pipeline instead of spending capacity on health checks or read-side polling.
