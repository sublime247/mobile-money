...
async function main() {
  ...
  const compressed = results.find(
    (r) => r.Scenario === "Transaction History (Compressed)",
  );
  const uncompressed = results.find(
    (r) => r.Scenario === "Transaction History (Uncompressed)",
  );
