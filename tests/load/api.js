 ...
 *   traffic_spike — spike 0 → 5 000 VUs   sudden peak-day traffic spike (5K req in 10s)
 *   spike_5k     — spike 0 → 5 000 VUs   alias for traffic_spike
 ...
 *   k6 run -e SCENARIO=traffic_spike tests/load/api.js
...
const PROFILES = {
  ...
  // Sudden peak-day traffic spike: ramps to 5000 VUs in 10 seconds and holds.
  traffic_spike: {
    executor: "ramping-vus",
    startVUs: 0,
    stages: [
    ...
    exec: "ingestTransactions",
    tags: { profile: "traffic_spike" },
  },

  // Alias for traffic_spike
  spike_5k: {
    executor: "ramping-vus",
    startVUs: 0,
    stages: [
...
const isHighVolume =
  SCENARIO === "ingestion_10k" ||
  SCENARIO === "spike_10k" ||
  SCENARIO === "traffic_spike" ||
  SCENARIO === "spike_5k";
const profileExec =
  activeProfile.exec ||
  (SCENARIO === "transactions" ? "transactionFocus" : "default");
...
export const options = {
  ...
  thresholds: {
    ...
    http_req_failed: [
      {
        threshold: isHighVolume ? "rate<0.10" : "rate<0.05",
    ...
    http_req_duration: [
      "p(50)<400",
      {
        threshold: isHighVolume ? "p(95)<5000" : "p(95)<2000",
        abortOnFail: isBreakpoint,
        delayAbortEval: "30s",
      },
      "p(99)<6000",
    ...
    tx_success_rate: [
      {
        threshold: isHighVolume ? "rate>0.85" : "rate>0.90",
        abortOnFail: isBreakpoint,
        delayAbortEval: "30s",
      },
    ],
...
export default function () {
  ...
  group("create_deposit", function () {
    ...
    const r = http.post(
      ...
      payload,
      params("deposit", {
        "Idempotency-Key": idempotencyKey(vuId, iter, "deposit"),
      }),
    );
...
export function transactionFocus() {
  ...
  if (iter % 3 !== 2) {
    ...
    const r = http.post(
      ...
      payload,
      params("withdraw", {
        "Idempotency-Key": idempotencyKey(vuId, iter, "withdraw"),
      }),
    );
...
export function ingestTransactions() {
  ...
  const r = http.post(
    ...
    payload,
    params(operation, {
      "Idempotency-Key": idempotencyKey(vuId, iter, operation),
    }),
  );
  const dur = Date.now() - start;
...
export function handleSummary(data) {
  ...
  const errorThreshold = isHighVolume ? 0.1 : 0.05;
  const latencyThreshold = isHighVolume ? 5000 : 2000;
  const successThreshold = isHighVolume ? 0.85 : 0.9;
...
function sizingRecommendation(tps, p95ms, errRate, peakVUs) {
  ...
  const errorCeiling = isHighVolume ? 0.1 : 0.05;
